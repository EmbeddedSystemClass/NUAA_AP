...
Transfer_Control_t *pxTransferControl = *ppxTransferControl;
	...
	{
		/* The transfer control structure does not exist.  Create it. */   /*分配内存空间,指针没有指向内存地址，动态分配*/
	...
	else
	{
		...
		switch( pxTransferControl->ucType )
		{
			case ioctlUSE_BLOCKED_TX	:
				...
				{
					Blocked_TxRx_State_t *pxBlockedTxState;
					pxBlockedTxState = ( Blocked_TxRx_State_t * ) ( pxTransferControl->pvTransferState );
					vPortFree( pxBlockedTxState );
				}
			...
			case ioctlUSE_BLOCKED_RX	:
				...
				{
					Blocked_TxRx_State_t *pxBlockedRxState;
					pxBlockedRxState = ( Blocked_TxRx_State_t * ) ( pxTransferControl->pvTransferState );
					vPortFree( pxBlockedRxState );
				}
				#endif /* ioconfigUSE_ZERO_COPY_TX */
				break;
