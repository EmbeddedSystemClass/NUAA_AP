 ...
 */
 ...
 /**
 *********************************************************************
 * Description           :Add can-bus driver
 *********************************************************************
 * 
...
Peripheral_Control_t *pxPeripheralControl = NULL;
	...
	if( xIndex < xNumberOfPeripherals )
	{
		...
		peripheral. */
		pxPeripheralControl = pvPortMalloc( sizeof( Peripheral_Control_t ) );/*�����ڴ淵��ָ��*/
...
portBASE_TYPE xReturn = pdFAIL, xCommandIsDeviceSpecific = pdFALSE;
	...
	switch( ulRequest )
	{
		case ioctlUSE_BLOCKED_TX:
			...
			#if ioconfigUSE_BLOCKED_TX == 1
			{
				/* The peripheral is going to use a	Polled_Tx_State_t
				...
				xReturn = xIOUtilsConfigureBlockedTx( pxPeripheralControl );
			}
			#endif /* ioconfigUSE_BLOCKED_TX */
			break;

		case ioctlUSE_BLOCKED_RX:

			#if ioconfigUSE_BLOCKED_RX == 1
			{
				/* The peripheral is going to use a	Polled_Tx_State_t
				structure for transmission.  This allows characters to be
				transmitted directly from the buffer supplied to the
				FreeRTOS_write() function. */
  				xReturn = xIOUtilsConfigureBlockedRx( pxPeripheralControl );
			}
			#endif /* ioconfigUSE_BLOCKED_RX */
			break;
		...
		case ioctlUSE_CIRCULAR_BUFFER_RX :
			...
			#if ioconfigUSE_CIRCULAR_BUFFER_RX == 1
			{
				...
				ulRequest = ioctlSTART_CIRCULAR_BUFFER_RX_DMA;
		...
		case ioctlSET_TX_TIMEOUT 	:
			...
			if( pxPeripheralControl->pxTxControl->ucType == ioctlUSE_BLOCKED_TX )
			{
				#if ( ioconfigUSE_BLOCKED_TX == 1 )
				{
					xIOUtilsSetBlockedTxRxTimeout( pxPeripheralControl, ( portTickType ) pvValue );
					xReturn = pdPASS;
				}
				#endif /* ( ioconfigUSE_BLOCKED_TX == 1 ) */
			}
			else if( pxPeripheralControl->pxTxControl->ucType == ioctlUSE_ZERO_COPY_TX )
			{
				#if ( ioconfigUSE_ZERO_COPY_TX == 1 )
				{
					xIOUtilsSetZeroCopyTxTimeout( pxPeripheralControl, ( portTickType ) pvValue );
					xReturn = pdPASS;
				}
				#endif /* ( ioconfigUSE_ZERO_COPY_TX == 1 ) */
			}
		...
		case ioctlSET_RX_TIMEOUT	:
			...
			if( pxPeripheralControl->pxRxControl->ucType == ioctlUSE_BLOCKED_RX )
			{
				#if ioconfigUSE_BLOCKED_RX == 1
				{
					xIOUtilsSetBlockedTxRxTimeout( pxPeripheralControl, ( portTickType ) pvValue );
