...
Zero_Copy_Tx_State_t *pxZeroCopyState;
	...
	if( pxPeripheralControl->pxTxControl != NULL )
	{
		...
		if( pxZeroCopyState != NULL )
		{
			...
			pxZeroCopyState->xWriteAccessMutex = xSemaphoreCreateBinary();
			pxZeroCopyState->xMode = 0U;
			pxZeroCopyState->xBlockTime = portMAX_DELAY;
...
{
Transfer_Control_t *pxTransferControlState = pxPeripheralControl->pxTxControl;
Zero_Copy_Tx_State_t *pxBlockedTxRxState;

	pxBlockedTxRxState = ( Zero_Copy_Tx_State_t * ) ( pxTransferControlState->pvTransferState );
